...
if __name__ == '__main__':
    ...
    axs[0].plot(y.max(axis=0), linewidth=1.5, color='black')
    axs[0].plot(y.min(axis=0), linewidth=1.5, color='black')
    axs[1].plot(np.log(y.max(axis=0)), linewidth=1.5, color='black')
    axs[1].plot(np.log(y.min(axis=0)), linewidth=1.5, color='black')
    ...
    axs[0].plot(y_sel.max(axis=0), linewidth=1.5, color='black')
    axs[0].plot(y_sel.min(axis=0), linewidth=1.5, color='black')
    axs[1].plot(np.log(y_sel.max(axis=0)), linewidth=1.5, color='black')
    axs[1].plot(np.log(y_sel.min(axis=0)), linewidth=1.5, color='black')
    ...
    y_sel_1 = y[y.count(axis=1)<=5]
    y_sel_2 = y[y.count(axis=1)<=10]
    y_sel_3 = y[y.count(axis=1)<20]
    y_sel_4 = y[y.count(axis=1)>5]
    y_sel_5 = y[y.count(axis=1)>10]
    y_sel_6 = y[y.count(axis=1)==20]

    #
    # Note: Here it would be better to use percentiles/quantiles instead of max.
    #       This should be more robust against outliers.
    #

    axs[0].plot(y_sel_2.max(axis=0), linewidth=1.5, color='tab:blue', linestyle='dashed')
    axs[0].plot(y_sel_2.mean(axis=0), linewidth=1.5, color='tab:blue')
    axs[0].plot(y_sel_2.min(axis=0), linewidth=1.5, color='tab:blue', linestyle='dashed')

    axs[0].plot(y_sel_5.max(axis=0), linewidth=1.5, color='tab:orange', linestyle='dashed')
    axs[0].plot(y_sel_5.mean(axis=0), linewidth=1.5, color='tab:orange')
    axs[0].plot(y_sel_5.min(axis=0), linewidth=1.5, color='tab:orange', linestyle='dashed')

    axs[0].plot(y.mean(axis=0), linewidth=1.5, color='black')

    axs[1].plot(np.log(y_sel_2.max(axis=0)), linewidth=1.5, color='tab:blue', linestyle='dashed')
    axs[1].plot(np.log(y_sel_2.mean(axis=0)), linewidth=1.5, color='tab:blue')
    axs[1].plot(np.log(y_sel_2.min(axis=0)), linewidth=1.5, color='tab:blue', linestyle='dashed')

    axs[1].plot(np.log(y_sel_5.max(axis=0)), linewidth=1.5, color='tab:orange', linestyle='dashed')
    axs[1].plot(np.log(y_sel_5.mean(axis=0)), linewidth=1.5, color='tab:orange')
    axs[1].plot(np.log(y_sel_5.min(axis=0)), linewidth=1.5, color='tab:orange', linestyle='dashed')

    axs[1].plot(np.log(y.mean(axis=0)), linewidth=1.5, color='black')
