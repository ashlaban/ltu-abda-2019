...
class Slice(SamplerBase):
    ...
    def sample(shape, pdf, x0=None, burnin=0.1,
        ...
        def _sample_inplace(pdf, x0, dst, start=0, stop=-1, ndims=-1, step_size=1.,
            ...
            for iSamp in range(start, stop):
                ...
                if ((iSamp % 100) == 0) and show_progress:
                    ...
                    print(show_progress_prefix, iSamp, '/', stop, '\u001b[1A')
            
            if show_progress:
                if show_progress_leave:
                    print(show_progress_prefix, iSamp+1, '/', stop)
                else:
                    print('\u001b[2K\u001b[1A')
        ...
        def _sample(pdf, x0, nsamples, ndims, step_size, dtype,
                    show_progress=False, show_progress_leave=False,
                    show_progress_prefix='Sample'):
            samples = np.empty(shape=(int(nsamples), int(ndims)), dtype=dtype)
            return _sample_inplace(pdf, x0, dst=samples, start=0,
                                   stop=nsamples, step_size=step_size,
